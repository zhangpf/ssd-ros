...
{  
  ...
  ros::NodeHandle nh;
  ros::Publisher pub = nh.advertise<std_msgs::String>("ssd/result", 10);
  std_msgs::String msg;
  msg.data = "OK";
  while (ros::ok()) {
...
void DetectionOutputLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  publish_ = detection_output_param.ros_publish();
  if (publish_) {
    boost::thread publish(publishThread); 
  }
}
...
void DetectionOutputLayer<Dtype>::Forward_cpu(
  ...
  int num_img = bottom[3]->num();
  clock_gettime(CLOCK_MONOTONIC, &finish);
  float elapsed = (finish.tv_sec - start.tv_sec);
  elapsed += (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
  float fps = num_img / elapsed;
  LOG(INFO) << "IMG: " << num_img << " FPS: " << fps;
  start = finish;
