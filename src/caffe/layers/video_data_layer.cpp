...
{  
  ...
  char ** argv = {};
    ...
    image_transport::TransportHints hintsRgb("compressed", ros::TransportHints(), nh);
    image_transport::Subscriber sub = it.subscribe("camera/rgb/image_color", 1, imageCallback, hintsRgb);
    ros::spin();
} 
...
void VideoDataLayer<Dtype>::DataLayerSetUp(
  ...
  } else if (video_type_ == VideoDataParameter_VideoType_ROS) {
    boost::thread subscribe(subscribeThread); 
    std::unique_lock<std::mutex> lock(m);
    while (!done) {
      cond_var.wait(lock);
    }
